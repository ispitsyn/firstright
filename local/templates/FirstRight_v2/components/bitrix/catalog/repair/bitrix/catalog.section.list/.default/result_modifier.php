...
if (0 < $arResult['SECTIONS_COUNT'])
{
    ...
    $first_sections_id = [];
    foreach ($arResult['SECTIONS'] as $itemSections) {
        if(strlen($itemSections['UF_FIRST_SECTION'])) {
            $first_sections_id[] = $itemSections['UF_FIRST_SECTION'];
        }
    }
    $first_sections_id = array_unique($first_sections_id);
    $arFilter = array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ID' => $first_sections_id); // выберет потомков без учета активности
    $rsSect = CIBlockSection::GetList(array('left_margin' => 'asc'),$arFilter);
    $first_sections = [];
    while ($arSect = $rsSect->GetNext())
    {
        $first_sections[$arSect['ID']] = $arSect;
    }
    foreach ($arResult['SECTIONS'] as &$itemSections) {
        $itemSections['FIRST_SECTION_LINK'] = $first_sections[$itemSections['UF_FIRST_SECTION']]['SECTION_PAGE_URL'];
    }
...
if($arResult['SECTION']['DEPTH_LEVEL'] === 0) {
    ...
    $res = CIBlock::GetArrayByID($arParams['IBLOCK_ID']);
