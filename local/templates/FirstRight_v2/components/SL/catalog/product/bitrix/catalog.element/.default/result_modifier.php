...
if (!empty($arResult['PROPERTIES']['CHARACTERISTICS_DESCRIPTION']['VALUE'])) {
    $path = CFile::GetPath($arResult['PROPERTIES']['CHARACTERISTICS_DESCRIPTION']['VALUE']);
    if(!empty($path)) {
        $fileName = \Bitrix\Main\Application::getDocumentRoot().$path;
        if(\Bitrix\Main\IO\File::isFileExists($fileName)) {
            $content =  \Bitrix\Main\IO\File::getFileContents($fileName);
            $content = json_decode($content, TRUE);
            $arResult['PROPERTIES']['CHARACTERISTICS_DESCRIPTION']['CONTENT'] = current($content);
        }
    }
}
