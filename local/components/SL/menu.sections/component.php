...
if($this->StartResultCache())
{
	...
	else
	{
		...
		while($arSection = $rsSections->GetNext())
		{
            ...
            $addParams[$arSection["ID"]]['SVG_ICON_CODE'] = $arSection["UF_SVG_ICON_CODE"];
            $addParams[$arSection["ID"]]['HIDDEN_MENU'] = $arSection["UF_HIDDEN_MENU"];
            if(!empty($arSection["UF_LABEL"])) {
				$rsRes= CUserFieldEnum::GetList(array(), array("ID" => $arSection["UF_LABEL"]));
				if($userFieldLabel = $rsRes->GetNext()) $addParams[$arSection["ID"]]['LABEL'] = ['CODE' => strtolower($userFieldLabel['XML_ID']), 'VALUE' => $userFieldLabel['VALUE']];
			}
		}
		$this->EndResultCache();
	}
...
foreach($arResult["SECTIONS"] as $arSection)
{
	...
	$aMenuLinksNew[$menuIndex++] = array(
		...
		array(
			...
			"SECTION_ICON" => $addParams[$arSection["ID"]]['SVG_ICON_CODE'],
			"LABEL" => $addParams[$arSection["ID"]]['LABEL'],
			"FROM_IBLOCK" => true,
			"IS_PARENT" => false,
			"DEPTH_LEVEL" => $arSection["DEPTH_LEVEL"],
		),
